...
class SyncEngine {
  ...
  private async pushChanges() {
    ...
    try {
      ...
      if (isAuthError) {
        console.log("[Sync] push returned 401 - sync server not configured, skipping");
        for (const item of pendingItems) {
          if (!item.id) continue;
          await db.syncQueue.put({
            ...item,
  ...
  private async pullChanges() {
    ...
    try {
      ...
      // Handle tombstones
      if (result?.tombstones && Array.isArray(result.tombstones)) {
      ...
      if (isAuthError) {
        console.log("[Sync] pull.query returned 401 - sync server not configured, skipping");
        await db.syncLogs.add({
          direction: "pull",
          status: "failed",
          message: "401 Unauthorized - sync skipped",
          createdAt: new Date().toISOString(),
        });
        return;
      }
