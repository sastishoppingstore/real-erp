          ...
          <CardContent className="space-y-4">
            ...
            {selectedCompany && (
              <div className="grid gap-4 rounded-lg border bg-slate-50 p-4 md:grid-cols-4">
                ...
                <div>
                  <p className="text-xs text-slate-500">{isAr ? "القالب" : "Theme"}</p>
                  <div className="mt-1 flex items-center gap-2">
                    <p className="font-semibold capitalize">{tenantTheme?.layoutTheme || "sidebar"}</p>
                    <button
                      onClick={() => {
                        const current = tenantTheme?.layoutTheme || "sidebar";
                        const next = current === "sidebar" ? "app_launcher" : current === "app_launcher" ? "launcher_theme" : "sidebar";
                        overrideTheme.mutate({ tenantId: selectedCompany.id, layoutTheme: next });
                      }}
                      disabled={overrideTheme.isPending}
