...
import { z } from "zod";
import { Building2, ChevronLeft, ChevronRight, Check, Globe, Shield, Lock, User, Mail, Phone, Users, Clock, Briefcase, Palette, Sparkles, Stethoscope, Wrench, Factory, Store, UtensilsCrossed, Hotel, GraduationCap, Truck, Home, Zap, LayoutDashboard, Grid as GridIcon } from "lucide-react";
...
const formSchema = z.object({
  ...
  layoutTheme: z.enum(["sidebar", "app_launcher", "launcher_theme"]).optional(),
...
export default function RegisterBusiness() {
  ...
  const onSubmit = async () => {
    ...
    const payload: Record<string, any> = {
      ...
      layoutTheme: values.layoutTheme || "sidebar",
      ...
      case 3: {
        ...
        return (
          <div className="space-y-6">
            ...
            <div>
              <h3 className="font-semibold mb-2">{isAr ? "اختر نمط لوحة التحكم" : "Choose Dashboard Layout"}</h3>
              <div className="grid grid-cols-2 gap-3">
                {[
                  { value: "sidebar" as const, label: "Classic Sidebar", labelAr: "قائمة جانبية كلاسيكية", icon: "LayoutDashboard" },
                  { value: "app_launcher" as const, label: "App Launcher (Icon Grid)", labelAr: "شاشة التطبيقات (أيقونات)", icon: "Grid" },
                  { value: "launcher_theme" as const, label: "Launcher Theme (3D)", labelAr: "ثيم المشغل (ثلاثي الأبعاد)", icon: "Rocket" },
                ].map((opt) => {
                  const selected = form.watch("layoutTheme") === opt.value;
                  return (
                    <button
                      key={opt.value}
                      type="button"
                      onClick={() => form.setValue("layoutTheme", opt.value)}
                      className={`flex flex-col items-center gap-2 rounded-xl border-2 p-4 text-center transition-all hover:shadow-md ${
                        selected ? "border-blue-500 bg-blue-50 shadow-sm" : "border-slate-200 hover:border-blue-300"
                      }`}
                    >
                      <div className={`rounded-full p-2 ${selected ? "bg-blue-100 text-blue-600" : "bg-slate-100 text-slate-600"}`}>
                        {opt.value === "sidebar" ? <LayoutDashboard className="size-6" /> : <GridIcon className="size-6" />}
                      </div>
