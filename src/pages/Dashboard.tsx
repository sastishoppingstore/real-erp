...
import AppLauncherHome from "@/components/themes/AppLauncher/AppLauncherHome";
import LauncherThemeDashboard from "@/components/LauncherThemeDashboard";
...
export default function Dashboard() {
  ...
  const { user } = useAuth();
  const { isAppLauncher, layoutTheme } = useLayoutTheme();
  ...
  if (layoutTheme === "launcher_theme") {
    return <LauncherThemeDashboard />;
  }
  if (isAppLauncher) {
    return <AppLauncherHome />;
  }
  ...
  const revenueByMonthArray = Array.isArray(revenueByMonth) ? revenueByMonth : [];
  const revenueData = revenueByMonthArray.map((r) => ({
        ...
        <Card>
          ...
          <CardContent>
            <div className="space-y-3">
              ...
              {(Array.isArray(recentInvoices) ? recentInvoices : []).slice(0, 5).map((inv) => (
                <Link
                  key={inv.id}
        ...
        <Card>
          ...
          <CardContent>
            <div className="space-y-3">
              ...
              {(Array.isArray(topCustomers) ? topCustomers : []).slice(0, 5).map((cust, i) => (
