...
export const workshopRouter = createRouter({
  ...
  estimateApprove: authedQuery
    ...
    .mutation(async ({ input }) => {
      const db = getDb();
      await db.update(workshopEstimates).set({ status: "approved", approvedAt: sql`(now())` }).where(eq(workshopEstimates.id, input.id));
      return { success: true };
    }),
  ...
  workshopStats: authedQuery
    .query(async ({ ctx }) => {
      ...
      const [completedToday] = await db.select({ count: sql<number>`count(*)` }).from(workshopJobCards).where(and(eq(workshopJobCards.tenantId, tenantId), eq(workshopJobCards.status, "completed"), gte(workshopJobCards.completionDate, sql`CURDATE()`)));
