...
export const superAdminRouter = createRouter({
  ...
  themes: {
    ...
    override: adminQuery
      .input(z.object({
        tenantId: z.number(),
        layoutTheme: z.enum(["sidebar", "app_launcher", "launcher_theme"]),
      }))
      .mutation(async ({ input, ctx }) => {
        const db = getDb();
        const old = await db.query.tenants.findFirst({
          where: eq(schema.tenants.id, input.tenantId),
        ...
        await createAuditLog({
          ...
          entityType: "tenant",
          entityId: input.tenantId,
          oldValues: { layoutTheme: old?.layoutTheme },
          newValues: { layoutTheme: input.layoutTheme },
        });
        return { success: true };
      }),
  },
