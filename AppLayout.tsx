...
const SidebarContent = memo(function SidebarContent({ collapsed, onNavigate }: { collapsed: boolean; onNavigate?: () => void }) {
  ...
  const { isAppLauncher, setLayoutTheme } = useLayoutTheme();
  ...
  return (
    <div dir={dir} className="flex h-full min-h-0 flex-col bg-slate-900 text-white">
      ...
      {/* Bottom Actions */}
      <div className={cn("border-t border-slate-700 p-3 space-y-1", collapsed && "p-2")}>
        {isAppLauncher && (
          <Button
            variant="ghost"
            size="sm"
            ...
            title={rtl ? "التبديل إلى القائمة الجانبية" : "Switch to Sidebar"}
          >
            <LayoutDashboard className="h-4 w-4 shrink-0" />
            {!collapsed && <span>{rtl ? "قائمة جانبية" : "Sidebar View"}</span>}
          </Button>
        )}
        <Button
          variant="ghost"
...
export default function AppLayout({ children }: { children: React.ReactNode }) {
  const { isAppLauncher, layoutTheme, setLayoutTheme } = useLayoutTheme();
  const [collapsed, setCollapsed] = useState(isAppLauncher);
  ...
  const isLauncherFullscreen = layoutTheme === "launcher_theme";
  ...
  return (
    <div dir={dir} className="flex h-dvh overflow-hidden bg-slate-50">
      {/* Desktop Sidebar (hidden in fullscreen Launcher Theme) */}
      <aside
        className={cn(
          "hidden transition-all duration-300 shrink-0 overflow-y-auto",
          isLauncherFullscreen ? "lg:hidden" : "h-dvh lg:block",
          collapsed ? "w-16" : "w-64"
        )}
      >
          ...
          <div className="order-3 w-full md:order-none md:w-[360px] lg:w-[460px]">
            <div className="relative cursor-pointer" onClick={() => setSearchOpen(true)}>
              ...
              <Input
                ...
                className="h-10 rounded-full border-white/20 bg-white/10 text-white placeholder:text-emerald-100/70 pl-9 pr-20 focus-visible:ring-emerald-400 cursor-pointer"
      ...
      <CommandDialog open={searchOpen} onOpenChange={setSearchOpen} title={rtl ? "بحث" : "Search"}>
        ...
        <CommandList>
          <CommandEmpty>{rtl ? "لا توجد نتائج" : "No results found."}</CommandEmpty>
          {menuGroups.map((group) => (
            <CommandGroup key={group.title} heading={group.label}>
              {group.items.map((item) => (
                <CommandItem
                  key={item.path}
