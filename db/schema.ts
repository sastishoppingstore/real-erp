...
export const tenants = mysqlTable("tenants", {
  ...
  plan: varchar("plan", { length: 100 }).default("free").notNull(),
  ...
  layoutTheme: mysqlEnum("layout_theme", ["sidebar", "app_launcher", "launcher_theme"]).default("launcher_theme").notNull(),
...
export const documentVersions = mysqlTable("document_versions", {
  ...
  index("doc_ver_tenant_idx").on(table.tenantId),
  index("doc_ver_doc_idx").on(table.tenantId, table.documentId),
]);
...
export const documentExpiryReminders = mysqlTable("document_expiry_reminders", {
  ...
  reminderDaysBefore: int("reminder_days_before").default(30),
  notifyUserIds: text("notify_user_ids"),
  createdBy: bigint("created_by", { mode: "number", unsigned: true }),
  status: varchar("status", { length: 20 }).default("active"),
  createdAt: timestamp("created_at").defaultNow().notNull(),
}, (table) => [
  index("doc_exp_tenant_idx").on(table.tenantId),
  index("doc_exp_status_idx").on(table.status),
]);
...
export const branches = mysqlTable("branches", {
  ...
  isActive: boolean("is_active").default(true),
  egsDevices: json("egs_devices"),
  invoiceCounter: int("invoice_counter").default(0),
  createdAt: timestamp("created_at").defaultNow().notNull(),
  updatedAt: timestamp("updated_at").defaultNow().notNull().$onUpdate(() => new Date()),
}, (table) => [
  index("branch_tenant_idx").on(table.tenantId),
]);
